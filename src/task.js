...
const runJob = async(task) => {
  ...
  try {
    // 计划任务到期就执行一个命令
    cqrs.bus.publishCommand({module: task.module, name: task.command, data: task.data});
  } catch (err) {
...
const createJob = async(task) => {
  ...
  const j = schedule.scheduleJob(task.module + '_' + task.name, task.type === 'cron'
    ? task.cron
...
const add = async(name, module, spec, command, data, description) => {
  ...
  if (exists) {
    ...
    await exists.save();
    // 这里不需要，用hook await createJob(exists);
  } else {
    await Task.create({
      name,
      module,
    ...
    // 这里不需要，用hook await createJob(task);
  }
}
